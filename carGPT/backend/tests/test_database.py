...
def postgres_container(test_db_name: str, test_db_user: str, test_db_password: str, test_db_port: int) -> Generator[Dict[str, Any], None, None]:
    """Session-scoped fixture that provides a PostgreSQL container."""
    with PostgresContainer("postgres:15", 
                          dbname=test_db_name,
                          username=test_db_user, 
                          password=test_db_password) as postgres:
...
def sample_ad_data() -> Dict[str, Any]:
    ...
    return {
        AdColumns.DATE_CREATED: datetime(2024, 1, 15, 12, 30, tzinfo=timezone.utc),
...
def sample_ad_data_minimal() -> Dict[str, Any]:
    ...
    return {
        AdColumns.DATE_CREATED: datetime.now(timezone.utc),
...
class TestDatabase:
    ...
    def test_insert_ad_mixed_valid_invalid_columns(self, database, sample_ad_data_minimal):
    ...
    def test_update_ad_protected_fields(self, database, sample_ad_data):
        ...
        update_data = {
            AdColumns.ID: 99999,
            AdColumns.INSERTION_TIME: datetime.now(timezone.utc),
